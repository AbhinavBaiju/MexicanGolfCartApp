...
// .wrangler/tmp/bundle-C8yPJI/checked-fetch.js
...
async function verifyProxySignature(request, secret) {
...
__name(verifyProxySignature, "verifyProxySignature");
...
async function linkAgreementSignature(db, shopDomain, orderId, noteAttributes, customerEmail) {
  ...
  ).bind(orderIdValue, emailValue, shopDomain, agreementSignatureId).run();
}
__name(linkAgreementSignature, "linkAgreementSignature");
function extractAgreementSignatureId(noteAttributes) {
...
__name(extractAgreementSignatureId, "extractAgreementSignatureId");
...
async function handleAgreementCurrent(env, shopDomain) {
  try {
    const shopStmt = await env.DB.prepare("SELECT id FROM shops WHERE shop_domain = ?").bind(shopDomain).first();
    if (!shopStmt) {
      return Response.json({ ok: false, error: "Shop not found" }, { status: 404 });
    ...
    const rentableProductIds = (productsRows.results ?? []).filter(isRecord2).map((row) => toNumber(row.product_id)).filter((value) => typeof value === "number");
    if (!agreementRow || !isRecord2(agreementRow)) {
...
__name(handleAgreementCurrent, "handleAgreementCurrent");
async function handleAgreementSign(request, env, shopDomain) {
  const body = await readJsonBody(request);
  if (!body) {
    return Response.json({ ok: false, error: "Invalid JSON body" }, { status: 400 });
  }
  ...
  try {
    ...
    await env.DB.prepare(
      `INSERT INTO signed_agreements (
            ...
            VALUES (?, ?, ?, ?, ?, ?, 'pending')`
    ).bind(signedId, shopDomain, agreementId, cartToken, signatureDataUrl, signedAt).run();
...
__name(handleAgreementSign, "handleAgreementSign");
function mapAgreementPublicRow(row) {
  ...
  if (version === null || pageNumber === null || x === null || y === null || width === null || height === null) {
    return null;
  }
  return {
    id,
...
__name(mapAgreementPublicRow, "mapAgreementPublicRow");
...
__name(isPngDataUrl, "isPngDataUrl");
...
async function handleAdminRequest(request, env) {
  ...
  if (segments.length >= 2 && segments[0] === "agreement") {
    if (segments.length === 2 && segments[1] === "current") {
      if (method === "GET") {
        return handleAgreementCurrent2(env, auth.shopDomain);
      }
    }
...
async function handleAgreementCurrent2(env, shopDomain) {
...
__name(handleAgreementCurrent2, "handleAgreementCurrent");
async function handleAgreementUpload(request, env, shopDomain, payload) {
  const body = await readJsonBody2(request);
  if (!body) {
    return jsonError("Invalid JSON body", 400);
  }
  const title = getOptionalString(body, "title") ?? null;
  const pdfUrl = getString2(body, "pdf_url");
  ...
  const xRaw = getOptionalNumber2(body, "x");
  const yRaw = getOptionalNumber2(body, "y");
  const widthRaw = getOptionalNumber2(body, "width");
  const heightRaw = getOptionalNumber2(body, "height");
  ...
  const maxVersion = isRecord3(versionRow) ? toNumber2(versionRow.max_version) ?? 0 : 0;
...
__name(handleAgreementUpload, "handleAgreementUpload");
async function handleAgreementPlacement(request, env, shopDomain) {
  const body = await readJsonBody2(request);
  if (!body) {
    return jsonError("Invalid JSON body", 400);
  }
  ...
  const x = getOptionalNumber2(body, "x");
  const y = getOptionalNumber2(body, "y");
  const width = getOptionalNumber2(body, "width");
  const height = getOptionalNumber2(body, "height");
  if (!pageNumber || x === void 0 || y === void 0 || width === void 0 || height === void 0 || !isNormalizedRect(x, y, width, height)) {
    return jsonError("Invalid placement fields", 400);
  }
  const target = agreementId ? await env.DB.prepare(
    "SELECT id FROM agreements WHERE shop_domain = ? AND id = ?"
  ).bind(shopDomain, agreementId).first() : await env.DB.prepare(
    "SELECT id FROM agreements WHERE shop_domain = ? AND active = 1 ORDER BY version DESC LIMIT 1"
  ).bind(shopDomain).first();
  if (!target || !isRecord3(target)) {
  ...
  await env.DB.prepare(
    `UPDATE agreements
         ...
         WHERE shop_domain = ? AND id = ?`
  ).bind(pageNumber, x, y, width, height, shopDomain, target.id).run();
  return Response.json({ ok: true });
}
__name(handleAgreementPlacement, "handleAgreementPlacement");
async function handleAgreementSignedList(request, env, shopDomain) {
  const url = new URL(request.url);
  const status = url.searchParams.get("status");
  ...
  const offset = clampInt(url.searchParams.get("offset"), 0, 0, 1e4);
  let sql = `SELECT s.id, s.agreement_id, s.cart_token, s.order_id, s.customer_email, s.signed_at, s.status,
  ...
  const rows = await env.DB.prepare(sql).bind(...bindings).all();
  const items = (rows.results ?? []).filter(isRecord3).map((row) => ({
    id: readStringField2(row, "id") ?? "",
    agreement_id: readStringField2(row, "agreement_id") ?? "",
    agreement_version: toNumber2(row.agreement_version) ?? 0,
    agreement_title: readStringField2(row, "agreement_title"),
    cart_token: readStringField2(row, "cart_token") ?? "",
    order_id: readStringField2(row, "order_id"),
    customer_email: readStringField2(row, "customer_email"),
    signed_at: readStringField2(row, "signed_at") ?? "",
    status: readStringField2(row, "status") ?? "pending"
  })).filter((item) => item.id && item.agreement_id);
  return Response.json({ ok: true, signed_agreements: items });
}
__name(handleAgreementSignedList, "handleAgreementSignedList");
async function handleAgreementSignedDetail(env, shopDomain, signedId) {
  const row = await env.DB.prepare(
  ...
  ).bind(shopDomain, signedId).first();
  if (!row || !isRecord3(row)) {
  ...
  const detail = {
    ...
    signed_at: readStringField2(row, "signed_at") ?? "",
    status: readStringField2(row, "status") ?? "pending",
    signature_png_base64: readStringField2(row, "signature_png_base64") ?? ""
  };
...
__name(handleAgreementSignedDetail, "handleAgreementSignedDetail");
async function handleAgreementActivate(env, shopDomain, agreementId) {
...
__name(handleAgreementActivate, "handleAgreementActivate");
function mapAgreementRow(row) {
  const id = readStringField2(row, "id");
  const pdfStorageKey = readStringField2(row, "pdf_storage_key");
  const pdfStorageType = readStringField2(row, "pdf_storage_type");
  if (!id || !pdfStorageKey || !pdfStorageType) {
    return null;
  }
  const version = toNumber2(row.version);
  const pageNumber = toNumber2(row.page_number);
  const x = toNumber2(row.x);
  const y = toNumber2(row.y);
  const width = toNumber2(row.width);
  const height = toNumber2(row.height);
  if (version === null || pageNumber === null || x === null || y === null || width === null || height === null) {
    return null;
  }
  return {
    id,
    version,
    active: Boolean(toNumber2(row.active)),
    title: readStringField2(row, "title"),
    pdf_url: pdfStorageKey,
    pdf_storage_type: pdfStorageType,
    pdf_sha256: readStringField2(row, "pdf_sha256"),
    page_number: pageNumber,
    x,
    y,
    width,
    height,
    created_at: readStringField2(row, "created_at") ?? "",
    created_by: readStringField2(row, "created_by")
  };
}
__name(mapAgreementRow, "mapAgreementRow");
function isNormalizedRect(x, y, width, height) {
...
__name(isNormalizedRect, "isNormalizedRect");
...
__name(isValidPdfUrl, "isValidPdfUrl");
function isAllowedPdfStorageType(value) {
  return value === "EXTERNAL" || value === "SHOPIFY_FILES";
}
__name(isAllowedPdfStorageType, "isAllowedPdfStorageType");
function isInvalidOptionalNumber(record, key, parsed) {
  return key in record && parsed === void 0;
}
__name(isInvalidOptionalNumber, "isInvalidOptionalNumber");
...
function readStringField2(row, key) {
  const value = row[key];
  if (typeof value === "string") {
    return value;
  }
...
__name(readStringField2, "readStringField");
...
function getOptionalPositiveInt(record, key) {
  if (!(key in record)) {
    return void 0;
  }
  const value = record[key];
...
__name(getOptionalPositiveInt, "getOptionalPositiveInt");
...
// .wrangler/tmp/bundle-C8yPJI/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-C8yPJI/middleware-loader.entry.ts
